...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ninja_") ; then
        NINJA_BUILD=$(echo -n $1 | sed -e 's/^ninja_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $NINJA_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        NINJA_BUILD=
    fi
    export NINJA_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${NINJA_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the NINJA model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NINJA_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.ninja.device | grep -q "$NINJA_BUILD");
    then
        ...
        echo "The connected device does not appear to be $NINJA_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.ninja.device | grep -q "$NINJA_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
